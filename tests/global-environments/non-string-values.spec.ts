...
test.describe('Global Environment Variables - Non-string Values', () => {
  test('should seed non-string globals via request and verify read-only + tooltip', async ({
    ...
    await test.step('Open environment selector and switch to Global tab (to create env)', async () => {
      await page.getByTestId('environment-selector-trigger').click();
      await page.getByTestId('env-tab-global').click();

      // Create a new global environment (if required by UI to view variables)
    ...
    await test.step('Close environments modal', async () => {
    ...
    await test.step('numericVar is read-only with tooltip', async () => {
      ...
      const numCM = numericRow.locator('.CodeMirror');
      await numCM.click();
      await page.keyboard.type('999');

      // Verify that typing into the input does not mutate the script-managed value.
      await expect(numericRow.locator('.CodeMirror-line').first()).toContainText(/170001/);
      ...
      await page.locator('#numericVar-disabled-info-icon').click();
      await expect(tooltip.first()).toBeVisible();
      await expect(tooltip.first()).toContainText('Non-string values set via scripts are read-only and can only be updated through scripts.');
    });

    await test.step('booleanVar is read-only', async () => {
      // This value is also set via post-script.
      await expect(booleanRow.locator('.CodeMirror-line').first()).toContainText(/true/);
      const boolCM = booleanRow.locator('.CodeMirror');
      await boolCM.click();
    ...
    await test.step('stringVar remains editable', async () => {
      ...
      const stringCM = stringRow.locator('.CodeMirror');
      await stringCM.click();
