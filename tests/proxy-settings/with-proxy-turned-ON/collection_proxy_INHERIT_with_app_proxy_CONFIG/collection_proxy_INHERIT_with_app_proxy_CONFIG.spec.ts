...
test.describe.serial('collection proxy INHERIT with app proxy CONFIG - `proxy server ON`', () => {
  test('developer mode - should succeed when proxy server is running', async ({ pageWithUserData: page }) => {
    ...
    // Collection proxy INHERIT uses app proxy CONFIG - should succeed (proxy working)
