...
test.describe.serial('collection proxy INHERIT with app proxy SYSTEM - `proxy server ON`', () => {
  test.beforeAll(() => {
  ...
  test('developer mode - should succeed when proxy server is running', async ({ pageWithUserData: page }) => {
    ...
    // Collection proxy INHERIT uses app proxy SYSTEM - should succeed (proxy working)
