...
test.describe.serial('collection-level proxy config object and with app-level proxy as config object', () => {
  test('developer mode', async ({ pageWithUserData: page }) => {
    ...
    await page.getByTitle('proxy-object').click();
    await page.getByRole('tab', { name: 'Proxy' }).click();
    await expect(page.getByRole('radio', { name: 'On' })).toBeChecked();
    ...
    await expect(page.getByRole('cell', { name: 'X-Bruno-Proxy-Collection' })).toBeVisible();
  });
