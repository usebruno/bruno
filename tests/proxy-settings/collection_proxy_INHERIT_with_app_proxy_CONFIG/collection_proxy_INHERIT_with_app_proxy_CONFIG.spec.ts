...
test.describe.serial('collection-level proxy inherit and with app-level proxy as config object', () => {
  test('developer mode', async ({ pageWithUserData: page }) => {
    ...
    // check if the app-level proxy is set to `on`
    await page.getByRole('button', { name: 'Open Preferences' }).click();
    await page.getByRole('tab', { name: 'Proxy' }).click();
    await expect(page.getByRole('radio', { name: 'On' })).toBeChecked();
    await page.locator('[data-test-id="modal-close-button"]').click();

    // check if the collection-level proxy is set to `inherit`
    await page.getByTitle('proxy-inherit').click();
    await page.getByRole('tab', { name: 'Proxy' }).click();
    await expect(page.getByRole('radio', { name: 'inherit' })).toBeChecked();
    ...
    await expect(page.getByRole('cell', { name: 'X-Bruno-Proxy-App' })).toBeVisible();
