...
test.describe.serial('collection proxy FALSE with app proxy CONFIG - `proxy server OFF`', () => {
  test('developer mode - should succeed when proxy server not running', async ({ pageWithUserData: page }) => {
    ...
    // Collection proxy FALSE ignores app proxy CONFIG - should succeed (no proxy used)
