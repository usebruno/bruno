...
test.describe.serial('collection proxy FALSE with app proxy SYSTEM - `proxy server OFF`', () => {
  test.beforeAll(() => {
  ...
  test('developer mode - should succeed when proxy server not running', async ({ pageWithUserData: page }) => {
    ...
    // Collection proxy FALSE ignores app proxy SYSTEM - should succeed (no proxy used)
