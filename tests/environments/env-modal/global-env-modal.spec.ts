...
test.describe('Global Environment Modal Tests', () => {
  test('should create collection and access environment modal', async ({ pageWithUserData: page, createTmpDir }) => {
    ...
    await page.getByLabel('Name').fill('env-modal-test-global');

    await page.getByLabel('Location').fill(await createTmpDir('env-modal-test-global'));
    await page.getByRole('button', { name: 'Create', exact: true }).click();

    // Wait for collection to appear and click on it
    await expect(page.locator('#sidebar-collection-name').filter({ hasText: 'env-modal-test-global' })).toBeVisible();
    await page.locator('#sidebar-collection-name').filter({ hasText: 'env-modal-test-global' }).click();
    await page.getByLabel('Safe Mode').check();
    await page.getByRole('button', { name: 'Save' }).click();

    // Open environment dropdown
    await page.locator('div.current-environment').click();
    await page.locator('.tab-button').filter({ hasText: 'Global' }).click();

    // Verify the Collection tab is active by default
    await expect(page.locator('.tab-button.active').filter({ hasText: 'Global' })).toBeVisible();
