...
test.describe('Request Description (YAML) - Write (Body: file)', () => {
  test('writes a multiline description to a file body row and persists it to the .yml file', async ({
    ...
    await test.step('Verify the description was persisted to the .yml file', async () => {
      const ymlPath = path.join(collectionFixturePath!, 'file-with-descriptions.yml');
