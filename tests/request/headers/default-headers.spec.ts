...
import { fillRequestHeaderName, fillRequestHeaderValue, readResponsePreviewBody } from '../../utils/request';
...
test('hides defaults and shows a flat editable request headers table', async ({ page, createTmpDir }) => {
  ...
  await test.step('Hide defaults', async () => {
    ...
    await expect(headers.addRow()).toBeVisible();
    await expect(headers.toggleDefaults()).toHaveText(`Show Defaults (${DEFAULT_HEADERS.length})`);
  });
  ...
  await test.step('Show defaults again', async () => {
    await headers.toggleDefaults().click();
