...
test.describe('Import Insomnia v5 Collection - Environment Import', () => {
  ...
  test('Import Insomnia v5 collection with nested environments and verify flattening', async ({
    ...
    await test.step('Test Base Environment - verify flattened keys', async () => {
      ...
      const baseUrlRow = baseUrlInput.locator('xpath=ancestor::tr');
      const authTokenRow = authTokenInput.locator('xpath=ancestor::tr');
      ...
      const userNameRow = userNameInput.locator('xpath=ancestor::tr');
      const userIdRow = userIdInput.locator('xpath=ancestor::tr');
      ...
      const userRoles0Row = userRoles0Input.locator('xpath=ancestor::tr');
      const userRoles1Row = userRoles1Input.locator('xpath=ancestor::tr');
      ...
      const configTimeoutRow = configTimeoutInput.locator('xpath=ancestor::tr');
      const configDebugRow = configDebugInput.locator('xpath=ancestor::tr');
    ...
    await test.step('Test Staging Environment - verify merging and overrides', async () => {
      ...
      const stagingBaseUrlRow = stagingBaseUrlInput.locator('xpath=ancestor::tr');
      ...
      const stagingAuthTokenRow = stagingAuthTokenInput.locator('xpath=ancestor::tr');
      ...
      const stagingUserNameRow = stagingUserNameInput.locator('xpath=ancestor::tr');
      const stagingUserIdRow = stagingUserIdInput.locator('xpath=ancestor::tr');
      ...
      const stagingConfigTimeoutRow = stagingConfigTimeoutInput.locator('xpath=ancestor::tr');
      const stagingConfigDebugRow = stagingConfigDebugInput.locator('xpath=ancestor::tr');
      // Assert: Staging overrides config.timeout with its own value
      await expect(stagingConfigTimeoutRow.locator('.CodeMirror-line').first()).toHaveText('60000');
      // Assert: Staging overrides config.debug with its own value
      await expect(stagingConfigDebugRow.locator('.CodeMirror-line').first()).toHaveText('false');
    });
    ...
    await test.step('Test Development Environment - verify new variables', async () => {
      ...
      const devBaseUrlRow = devBaseUrlInput.locator('xpath=ancestor::tr');
      const devAuthTokenRow = devAuthTokenInput.locator('xpath=ancestor::tr');
      ...
      const newFeatureEnabledRow = newFeatureEnabledInput.locator('xpath=ancestor::tr');
      const newFeatureVersionRow = newFeatureVersionInput.locator('xpath=ancestor::tr');
      ...
      const devUserRoles0Row = devUserRoles0Input.locator('xpath=ancestor::tr');
      // Assert: Development inherits user.roles[0] from base (not overridden in development)
      await expect(devUserRoles0Row.locator('.CodeMirror-line').first()).toHaveText('admin');
    });
