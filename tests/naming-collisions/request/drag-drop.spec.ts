...
/** The request row scoped to a specific collection's sidebar container. */
const requestInCollection = (page: Page, collectionName: string, requestName: string) =>
  page
...
test.describe('Naming collisions - drag/drop request', () => {
  ...
  test('dragging a request into a collection that has the same name suffixes the file', async ({ page, createTmpDir }) => {
    ...
    // Seed a colliding "login.bru" in the target directly on disk (avoids the
    // strict-locator issue of two UI-created requests sharing a name).
    ...
    await test.step('Drag "login" from the source collection onto the target collection', async () => {
      const source = requestInCollection(page, 'Drag Source', 'login');
      const target = page.locator('.collection-name').filter({ hasText: 'Drag Target' });
  ...
  test('dragging a request into a collection with no collision keeps its name', async ({ page, createTmpDir }) => {
    ...
    await test.step('Drag "login" onto the empty target collection', async () => {
      const source = requestInCollection(page, 'NC Source', 'login');
      const target = page.locator('.collection-name').filter({ hasText: 'NC Target' });
      await expect(source).toBeVisible();
      await source.dragTo(target);
    });
  ...
  test('reordering within the same collection does not create a copy', async ({ page, createTmpDir }) => {
    ...
    await test.step('Drag "beta" onto "alpha" (same-folder reorder)', async () => {
      const beta = requestInCollection(page, 'Reorder', 'beta');
      const alpha = requestInCollection(page, 'Reorder', 'alpha');
  ...
  test('cross-format move converts to .yml and suffixes on collision', async ({ page, createTmpDir }) => {
    ...
    await test.step('Drag the .bru "login" onto the yml collection', async () => {
      const source = requestInCollection(page, 'Xfmt Source', 'login');
      const target = page.locator('.collection-name').filter({ hasText: 'Xfmt Target' });
      await expect(source).toBeVisible();
      await source.dragTo(target);
    });
  ...
  test('an open tab follows a request moved into a folder (identity preserved)', async ({ page, createTmpDir }) => {
    ...
    await locators.sidebar.folder('Auth').dblclick(); // expand the (empty) folder
    ...
    await test.step('Open the request, then drag it into the "Auth" folder', async () => {
      await locators.sidebar.request('login').first().dblclick();
      await expect(locators.tabs.requestTab('login')).toBeVisible({ timeout: 5000 });
      await locators.sidebar.request('login').first().dragTo(locators.sidebar.folder('Auth'));
    });
