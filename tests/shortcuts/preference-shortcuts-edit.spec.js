...
test.describe('Preferences - Keybindings Editor', () => {
  ...
  test.describe('FUNCTIONAL: Clone Item', () => {
    ...
    test('should clone folder using default Cmd/Ctrl+D', async ({ page, createTmpDir }) => {
      ...
      await getResetBtn(page, 'cloneItem').click();
      await page.waitForTimeout(500);

      // Verify it's reset to default
      const input = getInput(page, 'cloneItem');
      const resetValue = await input.inputValue();
      expect(resetValue).toContain('d');
      expect(resetValue).not.toContain('shift');
  ...
  test.describe('FUNCTIONAL: Copy and Paste Item', () => {
    ...
    test('should copy and paste request using customized Alt+C+R and Alt+V+R', async ({ page, createTmpDir }) => {
      ...
      let row = page.getByTestId('keybinding-row-copyItem');
      await row.hover();
      ...
      row = page.getByTestId('keybinding-row-pasteItem');
      await row.hover();
    ...
    test('should copy and paste folder using default Cmd/Ctrl+C and Cmd/Ctrl+V', async ({ page, createTmpDir }) => {
      // Close all collections first for clean state
      await closeAllCollections(page);
      ...
      // Reset copyItem and pasteItem keybindings to default
      await openKeybindingsTab(page);
      ...
      // Verify they're reset to default
      let input = getInput(page, 'copyItem');
      let resetValue = await input.inputValue();
      expect(resetValue).toContain('c');
      expect(resetValue).not.toContain('alt');

      input = getInput(page, 'pasteItem');
      resetValue = await input.inputValue();
      expect(resetValue).toContain('v');
      expect(resetValue).not.toContain('alt');
      ...
      // Now create collection and folder
      const collectionPath = await createTmpDir('copy-paste-folder-default');
      await createCollection(page, 'test-collection-copy-paste-folder-default', collectionPath);

      // Create a folder
      await createFolder(page, 'original-folder', 'test-collection-copy-paste-folder-default', true);
      //   await page.waitForTimeout(500);
      ...
      // Press Cmd/Ctrl+C to copy folder
      await page.keyboard.press(`${modifier}+KeyC`);
      await page.waitForTimeout(300);

      // Press Cmd/Ctrl+V to paste folder
      await page.keyboard.press(`${modifier}+KeyV`);
      await page.waitForTimeout(1000);
  ...
  test.describe('FUNCTIONAL: Move Tab Left', () => {
    test('should move tab left using default Cmd/Ctrl+Shift+PageUp', async ({ page, createTmpDir }) => {
      // Close all collections first for clean state
      await closeAllCollections(page);
      ...
      // Press Cmd/Ctrl+Shift+PageUp to move tab left
      await page.keyboard.press(`${modifier}+Shift+PageUp`);
      await page.waitForTimeout(500);
  ...
  test.describe('FUNCTIONAL: Move Tab Right', () => {
    test('should move tab right using default Cmd/Ctrl+Shift+PageDown', async ({ page, createTmpDir }) => {
      // Close all collections first for clean state
      await closeAllCollections(page);
      ...
      // Press Cmd/Ctrl+Shift+PageDown to move tab right
      await page.keyboard.press(`${modifier}+Shift+PageDown`);
      await page.waitForTimeout(500);
