...
test.describe('Create HTTP Requests', () => {
  test('Create HTTP request at collection root level', async ({ pageWithUserData: page }) => {
    ...
    await test.step('Verify HTTP request was created at collection root', async () => {
      ...
      const requestItem = page.locator('.collection-item-name').filter({ hasText: 'Root HTTP Request' });
      await expect(requestItem).toBeVisible();
      await requestItem.click();
      await expect(page.locator('.request-tab .tab-label')).toContainText('Root HTTP Request');
    });

    await test.step('Clean up', async () => {
      await page.locator('.collection-item-name').filter({ hasText: 'Root HTTP Request' }).click({ button: 'right' });
  ...
  test('Create HTTP request within folder1', async ({ pageWithUserData: page }) => {
    ...
    await test.step('Verify HTTP request was created within folder1', async () => {
      ...
      const requestItem = page.locator('.collection-item-name').filter({ hasText: 'Folder HTTP Request' });
      await expect(requestItem).toBeVisible({ timeout: 1000 });
      await requestItem.click();
      await expect(page.locator('.request-tab .tab-label')).toContainText('Folder HTTP Request');
    ...
    await test.step('Clean up', async () => {
      await page.locator('.collection-item-name').filter({ hasText: 'Folder HTTP Request' }).click({ button: 'right' });
      await page.locator('.dropdown-item').filter({ hasText: 'Delete' }).click();
      await page.getByRole('button', { name: 'Delete', exact: true }).click();
    });
  });
