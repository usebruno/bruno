...
function deriveKeyAndIv(password: string, keyLength: number, ivLength: number): KeyIvPair {
...
function aes256Encrypt(data: string): string {
...
function aes256Decrypt(data: string): string {
  const rawKey = machineIdSync();
...
function safeStorageEncrypt(str: string): string {
  const encryptedStringBuffer = safeStorage.encryptString(str);
  // Convert the encrypted buffer to a hexadecimal string
  return encryptedStringBuffer.toString('hex');
}

function safeStorageDecrypt(str: string): string {
  ...
  return decryptedStringBuffer.toString();
}
