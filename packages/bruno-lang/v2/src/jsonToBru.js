...
const getDescriptionSuffix = (item) => {
  ...
  return ' @description("' + escapeDescription(desc) + '")';
};
...
`;
  ...
  if (params && params.length) {
    ...
    if (pathParams.length) {
      ...
      bru += `\n${indentString(pathParams.map((item) => `${item.name}: ${getValueString(item.value)}${getDescriptionSuffix(item)}`).join('\n'))}`;
...
`;
      ...
      bru += `\n${indentString(
        multipartForms
          .map((item) => {
            ...
            const descSuffix = getDescriptionSuffix(item);

            if (item.type === 'text') {
              return `${enabled}${getKeyString(item.name)}: ${getValueString(item.value)}${contentType}${descSuffix}`;
            }
            ...
            if (item.type === 'file') {
              ...
              return `${enabled}${getKeyString(item.name)}: ${value}${contentType}${descSuffix}`;
            }
          })
          .join('\n')
      )}`;
  ...
  if (reqvars && reqvars.length) {
    ...
    if (varsEnabled.length) {
      bru += `\n${indentString(varsEnabled.map((item) => `${item.name}: ${getValueString(item.value)}${getDescriptionSuffix(item)}`).join('\n'))}`;
    }

    if (varsLocalEnabled.length) {
      bru += `\n${indentString(varsLocalEnabled.map((item) => `@${item.name}: ${getValueString(item.value)}${getDescriptionSuffix(item)}`).join('\n'))}`;
    }

    if (varsDisabled.length) {
      bru += `\n${indentString(varsDisabled.map((item) => `~${item.name}: ${getValueString(item.value)}${getDescriptionSuffix(item)}`).join('\n'))}`;
    }

    if (varsLocalDisabled.length) {
      bru += `\n${indentString(varsLocalDisabled.map((item) => `~@${item.name}: ${getValueString(item.value)}${getDescriptionSuffix(item)}`).join('\n'))}`;
    }
  ...
  if (assertions && assertions.length) {
    ...
    if (enabled(assertions).length) {
      bru += `\n${indentString(
        enabled(assertions)
          .map((item) => `${item.name}: ${getValueString(item.value)}${getDescriptionSuffix(item)}`)
          .join('\n')
      )}`;
    }
