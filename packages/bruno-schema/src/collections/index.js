...
const authOAuth1Schema = Yup.object({
  ...
  signatureMethod: Yup.string()
    .oneOf(['HMAC-SHA1', 'HMAC-SHA256', 'HMAC-SHA512', 'RSA-SHA1', 'RSA-SHA256', 'RSA-SHA512', 'PLAINTEXT'])
    .nullable(),
  parameterTransmission: Yup.string()
    .oneOf(['authorization_header', 'request_body', 'query_param'])
    .nullable(),
  ...
  credentialsId: Yup.string().nullable()
})
  .noUnknown(true)
  .strict();
...
const authSchema = Yup.object({
  mode: Yup.string()
    .oneOf(['inherit', 'none', 'awsv4', 'basic', 'bearer', 'digest', 'ntlm', 'oauth2', 'oauth1', 'wsse', 'apikey'])
    .required('mode is required'),
