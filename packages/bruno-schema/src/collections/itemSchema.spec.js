...
describe("Item Schema Validation", () => {
  ...
  it("item schema must throw an error if name is missing", async () => {
    ...
    return Promise.all([expect(itemSchema.validate(item)).rejects.toEqual(validationErrorWithMessages("name is required"))]);
  });
  ...
  it("item schema must throw an error if name is empty", async () => {
    ...
    return Promise.all([expect(itemSchema.validate(item)).rejects.toEqual(validationErrorWithMessages("name must be atleast 1 characters"))]);
  });
  ...
  it("item schema must throw an error if request is not present when item-type is http-request", async () => {
    ...
    return Promise.all([expect(itemSchema.validate(item)).rejects.toEqual(validationErrorWithMessages("request is required when item-type is request"))]);
  });
