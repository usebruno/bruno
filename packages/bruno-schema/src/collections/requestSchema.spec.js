...
describe("Request Schema Validation", () => {
  ...
  it("request schema must throw an error of method is invalid", async () => {
    ...
    return Promise.all([
      expect(requestSchema.validate(request)).rejects.toEqual(validationErrorWithMessages("method must be one of the following values: GET, POST, PUT, DELETE, PATCH, HEAD")),
    ]);
  });
});
