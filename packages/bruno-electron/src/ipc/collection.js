...
const registerRendererEventHandlers = (mainWindow, watcher) => {
  ...
  // On-demand parse: full parse a single request file when user clicks it (deferred-parse mode)
  ipcMain.handle('renderer:parse-request-on-demand', async (event, { collectionUid, pathname, collectionPath }) => {
    const file = {
      meta: {
        collectionUid,
        pathname,
    ...
    try {
      ...
      // Strip worker timing metadata if present
  ...
  // Bulk parse: parse all deferred request files using Phase 1 pipeline (parallel workers + loading state)
  // Used by the runner before execution — reuses the same worker pool and IPC flow as collection mount
  ipcMain.handle('renderer:bulk-parse-deferred-requests', async (event, { collectionUid, collectionPath, pathnames }) => {
    const format = getCollectionFormat(collectionPath);
    console.log(`[BULK-PARSE] Starting bulk parse of ${pathnames.length} deferred files for collection ${collectionUid}`);

    // Signal loading state so renderer batches the incoming addFile events (Phase 1 batching)
    mainWindow.webContents.send('main:collection-loading-state-updated', {
      collectionUid,
      isLoading: true
    });

    try {
      const parsePromises = pathnames.map(async (pathname) => {
        const file = {
          meta: {
            collectionUid,
            pathname,
        ...
        try {
          ...
          console.error(`[BULK-PARSE] Failed to parse: ${pathname}`, err);
        }
      });

      await Promise.all(parsePromises);
      console.log(`[BULK-PARSE] Completed parsing ${pathnames.length} files`);
    } finally {
      // Signal loading complete — triggers final batch flush on renderer
      mainWindow.webContents.send('main:collection-loading-state-updated', {
        collectionUid,
        isLoading: false
      });
