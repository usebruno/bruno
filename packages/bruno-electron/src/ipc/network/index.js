    ...
    async (event, folder, collection, environment, runtimeVariables, recursive, delay) => {
      ...
      const sendRequest = async (requestConfig, callback) => {
        return new Promise(async (resolve, reject) => {
          const _configuredRequestItem = getConfiguredRequestItem({ requestConfig });
          try {
            let response = await runRequest({ item: _configuredRequestItem, collection, envVars, processEnvVars, runtimeVariables, runInBackground: true, configuredRequestItem: _configuredRequestItem });
            if (callback) { callback(null, response); resolve(); }
            else { resolve(response); }
          }
          catch(error) {
            if (callback) { callback({ error: true, message: error?.message }, null); resolve(); }
            else { reject(error); }
          }
        });
      }
