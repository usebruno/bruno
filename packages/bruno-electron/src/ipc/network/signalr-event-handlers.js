...
const registerSignalrEventHandlers = (mainWindow) => {
  ...
  ipcMain.handle('renderer:signalr:start-connection', async (event, { requestId, collectionUid, url, auth }) => {
    try {
      ...
      // This replaces the need to register individual handlers via renderer:signalr:register-handler.
      // We never call the original _invokeClientMethod so there are no duplicate events.
