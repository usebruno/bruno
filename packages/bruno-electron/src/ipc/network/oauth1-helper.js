...
const addOAuth1Authorization = async (request, collectionPath) => {
  ...
  const shouldIncludeRequestBodyParams = (request) => {
    return request.headers['content-type'] === 'application/x-www-form-urlencoded' && request.method === 'POST';
  }

  // https://oauth.net/core/1.0/#anchor13
  // 7.  Accessing Protected Resources
  const addAuthorizationToRequest = (request, accessToken, accessTokenSecret) => {
    let requestData;
    if(shouldIncludeRequestBodyParams(request)) {
      requestData = {
        url: request.url,
    ...
    const authHeader = oauth.toHeader(oauth.authorize(requestData, signingToken));
  ...
  let { accessToken, accessTokenSecret, verifier } = request.oauth1;
  ...
  try {
    ...
    addAuthorizationToRequest(request, accessToken, accessTokenSecret);
  } catch (error) {
