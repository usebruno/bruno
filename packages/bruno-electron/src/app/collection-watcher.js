...
class CollectionWatcher {
  ...
  // Add watcher for scratch directory (workspace-level scratch requests)
  // Similar to addTempDirectoryWatcher but sends workspace-specific events
  addScratchDirectoryWatcher(win, tempDirectoryPath, workspaceUid) {
    if (this.watchers[tempDirectoryPath]) {
      this.watchers[tempDirectoryPath].close();
    }
    ...
    // Function to process and send scratch request file
    const processScratchFile = async (pathname, eventType) => {
      // Skip metadata.json
      if (path.basename(pathname) === 'metadata.json') {
        return;
      }
      ...
      try {
        ...
        if (eventType === 'unlink') {
          // For unlink, we just need the pathname
          file.data = {
            uid: getRequestUid(pathname),
            name: path.basename(pathname).replace('.bru', '')
          };
          win.webContents.send('main:scratch-request-removed', file);
        } else {
          // For add/change, parse the file content
          const content = fs.readFileSync(pathname, 'utf8');
          const fileStats = fs.statSync(pathname);

          file.data = await parseRequest(content, { format: 'bru' });
          file.size = sizeInMB(fileStats?.size);
          hydrateRequestWithUuid(file.data, pathname);

          if (eventType === 'add') {
            win.webContents.send('main:scratch-request-added', file);
          } else if (eventType === 'change') {
            win.webContents.send('main:scratch-request-changed', file);
          }
        }
      } catch (error) {
        console.error(`Error processing scratch file ${pathname}:`, error);
      }
    };

    watcher
      .on('add', (pathname) => processScratchFile(pathname, 'add'))
      .on('change', (pathname) => processScratchFile(pathname, 'change'))
      .on('unlink', (pathname) => processScratchFile(pathname, 'unlink'))
      .on('error', (error) => {
        console.error(`An error occurred in the scratch directory watcher for: ${tempDirectoryPath}`, error);
      });

    this.watchers[tempDirectoryPath] = watcher;
  }

  // Remove scratch directory watcher
  removeScratchDirectoryWatcher(tempDirectoryPath) {
    if (this.watchers[tempDirectoryPath]) {
      this.watchers[tempDirectoryPath].close();
      delete this.watchers[tempDirectoryPath];
    }
