...
import { fromOpenCollectionGraphQLSubscriptionItem, toOpenCollectionGraphQLSubscriptionItem } from './graphql-subscription';
import type {
  BrunoItem
} from '../types';
...
const getItemType = (item: OCItem): string => {
  ...
  if ('graphqlSubscription' in item && item.graphqlSubscription) {
...
export const fromOpenCollectionItem = (item: unknown, parseFolder: (folder: unknown) => BrunoItem): BrunoItem | null => {
  ...
  switch (itemType) {
    ...
    case 'graphql-subscription':
      return fromOpenCollectionGraphQLSubscriptionItem(item as Parameters<typeof fromOpenCollectionGraphQLSubscriptionItem>[0]);
    case 'folder':
...
export const toOpenCollectionItem = (item: BrunoItem, stringifyFolder: (folder: BrunoItem) => unknown): unknown | null => {
  switch (item.type) {
    ...
    case 'graphql-subscription-request':
      return toOpenCollectionGraphQLSubscriptionItem(item);
    case 'folder':
...
export { fromOpenCollectionGraphQLSubscriptionItem, toOpenCollectionGraphQLSubscriptionItem } from './graphql-subscription';
