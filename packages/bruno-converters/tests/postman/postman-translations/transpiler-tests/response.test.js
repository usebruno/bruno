...
describe('Response Translation', () => {
  ...
  it('should handle accessing nested properties on response objects', () => {
    ...
    expect(translatedCode).toContain('bru.getEnvVarList().set("userId", data.user.id);');
  });
  ...
  it('should transform response data with array destructuring', () => {
    ...
    expect(translatedCode).toContain('bru.getEnvVarList().set("userId", id);');
  });
  ...
  it('should handle response in complex conditionals', () => {
    ...
    expect(translatedCode).toContain('const data = res.getBody();');
    expect(translatedCode).toContain('bru.getEnvVarList().set("authToken", data.token);');
  ...
  it('should handle response processing with try-catch', () => {
    ...
    expect(translatedCode).toContain('const data = res.getBody();');
    expect(translatedCode).toContain('bru.getEnvVarList().set("userData", JSON.stringify(data.user));');
    expect(translatedCode).toContain('const text = JSON.stringify(res.getBody());');
    expect(translatedCode).toContain('bru.getEnvVarList().set("rawResponse", text);');
  });
  ...
  it('should handle JSON path style access to response data', () => {
    ...
    expect(translatedCode).toContain('bru.getEnvVarList().set("userId", userId);');
  });
  ...
  it('should handle template literals with response data', () => {
    ...
    expect(translatedCode).toContain('bru.getEnvVarList().set("welcomeMessage", welcomeMessage);');
  });
  ...
  it('should handle response processing in arrow functions', () => {
    ...
    expect(translatedCode).toContain('bru.getEnvVarList().set("itemIds", JSON.stringify(itemIds));');
  });
  ...
  it('should handle complex inline operations with response data', () => {
    ...
    expect(translatedCode).toContain('bru.getEnvVarList().set("totalValue", totalValue);');
    expect(translatedCode).toContain('bru.getEnvVarList().set("highValueItemCount", highValueItems.length);');
  });
