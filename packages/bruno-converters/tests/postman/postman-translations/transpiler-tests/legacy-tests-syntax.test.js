...
describe('Legacy Tests[] Syntax Translation', () => {
  ...
  it('should handle tests[] with pm.environment variables', () => {
    ...
    expect(translatedCode).toBe(`
        test("Response matches environment variable", function() {
                expect(Boolean(res.getBody().id === bru.getEnvVarList().get("expectedId"))).to.be.true;
        });`);
  });
  ...
  it('should handle nested pm objects in tests[] assignments', () => {
    ...
    expect(translatedCode).toContain('res.getBody().items.length === bru.getVarList().get("expectedCount")');
  });
  ...
  it('should handle complex real-world tests[] example', () => {
    ...
    expect(translatedCode).toContain('const expectedItems = parseInt(bru.getEnvVarList().get("expectedItemCount"));');
    ...
    expect(translatedCode).toContain('const targetItem = response.items.find(item => item.name === bru.getVarList().get("targetItemName"));');
    expect(translatedCode).toContain('bru.getEnvVarList().set("targetItemId", targetItem.id);');
  });
});
