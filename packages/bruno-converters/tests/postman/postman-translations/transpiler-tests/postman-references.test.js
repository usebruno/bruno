...
describe('Postman to PM References Conversion', () => {
  ...
  it('should convert basic postman references to pm', () => {
    ...
    expect(translatedCode).toBe('bru.getEnvVarList().set("key", "value");');
  ...
  it('should convert postman variable access to pm', () => {
    ...
    const translatedCode = translateCode(code);
    expect(translatedCode).toBe('const value = bru.getEnvVarList().get("key");');
  });
  ...
  it('should handle postman variable assignments', () => {
    ...
    `;
    const translatedCode = translateCode(code);
    expect(translatedCode).toContain('const envVar = bru.getEnvVarList().get("apiKey");');
  ...
  it('should handle postman references in object destructuring', () => {
    ...
    const translatedCode = translateCode(code);
    expect(translatedCode).toContain('bru.getEnvVarList().set("key", "value");');
  });
  ...
  it('should handle postman references in control flow statements', () => {
    ...
    const translatedCode = translateCode(code);
    expect(translatedCode).toContain('if (bru.getEnvVarList().get("isProduction") === "true") {');
    expect(translatedCode).toContain('const apiUrl = bru.getEnvVarList().get("prodUrl");');
    expect(translatedCode).toContain('bru.setNextRequest("Production Flow");');
    expect(translatedCode).toContain('const apiUrl = bru.getEnvVarList().get("devUrl");');
  ...
  it('should handle legacy postman response methods', () => {
    ...
    expect(translatedCode).toContain('bru.getEnvVarList().set("lastResponseCode", responseCode);');
  });
