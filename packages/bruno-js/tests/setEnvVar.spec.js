...
describe('Bru.deleteEnvVar', () => {
  ...
  test('removes variable from envVariables', () => {
    const bru = makeBru();
    bru.setEnvVar('key', 'value');
    expect(bru.envVariables.key).toBe('value');

    bru.deleteEnvVar('key');
    expect(bru.envVariables.key).toBeUndefined();
  });

  test('removes variable from persistentEnvVariables', () => {
    const bru = makeBru();
    bru.setEnvVar('key', 'value', { persist: true });
    expect(bru.persistentEnvVariables.key).toBe('value');
