...
import { sendRequest, saveRequest, saveAllRequestsInCollection } from 'providers/ReduxStore/slices/collections/actions';
...
export const HotkeysProvider = (props) => {
  ...
  // New hotkey for saving all requests in the active collection
  useEffect(() => {
    Mousetrap.bind(['command+shift+s', 'ctrl+shift+s'], (e) => {
      ...
      if (activeTab) {
        ...
        if (activeCollection) {
          dispatch(saveAllRequestsInCollection(activeCollection.uid));
        }
      }
