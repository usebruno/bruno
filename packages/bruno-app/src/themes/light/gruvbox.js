...
// Official Gruvbox Light palette (https://github.com/morhetz/gruvbox)
// Provides three contrast variants: soft, medium, hard.
// Only background tokens differ between variants; all hues, text, accent,
// and syntax tokens are shared.
...
// Background tokens per contrast variant. Each row picks where on the gruvbox
// bg ladder the app's BASE sits. SURFACE0/1/2 are always bg1/bg2/bg3 (darker
// than any light BASE), and MANTLE/CRUST sit between BASE and SURFACE0 for
// visual layering.
const backgroundsByVariant = {
  hard: {
    BASE: '#f9f5d7', // bg0_h
    MANTLE: '#f9f5d7',
    CRUST: '#fbf1c7', // bg0
...
const createPalette = (variant) => {
  const palette = {
    ...sharedPalette,
    background: backgroundsByVariant[variant]
  };
...
const createGruvboxLightTheme = (variant) => {
  ...
  return {
    ...
    codemirror: {
      bg: palette.background.MANTLE,
      border: palette.background.MANTLE,
      placeholder: {
        color: palette.text.SUBTEXT0,
        opacity: 0.6
      },
...
export const gruvboxLightSoft = createGruvboxLightTheme('soft');
export const gruvboxLightMedium = createGruvboxLightTheme('medium');
export const gruvboxLightHard = createGruvboxLightTheme('hard');
