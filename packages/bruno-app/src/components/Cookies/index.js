...
const ClearDomainCookiesModal = ({ onClose, domain, onClear }) => {
  ...
  return (
    <Modal onClose={onClose} handleCancel={onClose} title={t('COOKIES.CLEAR_DOMAIN_COOKIES')} hideFooter={true}>
      <div className="flex items-center font-normal">
        <IconAlertTriangle size={32} strokeWidth={1.5} className="warning-icon" />
        <h1 className="ml-2 text-lg font-medium">{t('COMMON.HOLD_ON')}</h1>
      </div>
      <div className="font-normal mt-4">
        {t('COOKIES.CLEAR_DOMAIN_COOKIES_CONFIRM', { domain })}
      </div>
...
const DeleteCookieModal = ({ onClose, cookieName, onDelete }) => {
  const { t } = useTranslation();
  return (
    <Modal onClose={onClose} handleCancel={onClose} title={t('COOKIES.DELETE_COOKIE')} hideFooter={true}>
      <div className="flex items-center font-normal">
        <IconAlertTriangle size={32} strokeWidth={1.5} className="warning-icon" />
        <h1 className="ml-2 text-lg font-medium">{t('COMMON.HOLD_ON')}</h1>
      </div>
...
const CollectionProperties = ({ onClose }) => {
  ...
  const clearDomainCookiesAction = () => {
    dispatch(deleteCookiesForDomain(domainToClear))
      .then(() => {
        toast.success(t('COOKIES.DOMAIN_COOKIES_CLEARED_SUCCESSFULLY'));
      })
      .catch((err) => console.log(err) && toast.error(t('COOKIES.FAILED_CLEAR_DOMAIN_COOKIES')));
  ...
  const deleteCookieAction = () => {
    if (cookieToDelete) {
      ...
      dispatch(deleteCookie(domain, path, key))
        .then(() => {
          toast.success(t('COOKIES.COOKIE_DELETED_SUCCESSFULLY'));
        })
        .catch((err) => console.log(err) && toast.error(t('COOKIES.FAILED_DELETE_COOKIE')));
    }
      ...
      <Modal
        ...
        customHeader={shouldShowHeader ? (
          <StyledWrapper className="header flex items-center justify-between w-full">
            <h2 className="text-xs font-medium">{t('COMMON.COOKIES')}</h2>
            <input
              type="search"
              placeholder={t('COOKIES.SEARCH_BY_DOMAIN')}
              value={searchText || ''}
        ...
        <StyledWrapper>
          {!cookies || !cookies.length ? (
            ...
            <div className="flex items-center justify-center flex-col">
              ...
              <h2 className="text-lg font-medium mt-4">{t('COOKIES.NO_COOKIES_FOUND')}</h2>
              <p className="empty-text mt-2">{t('COOKIES.ADD_COOKIES_GET_STARTED')}</p>
              <Button
                type="submit"
                size="sm"
                className="mt-8"
            ...
            <div className="flex items-center justify-center flex-col">
              <IconSearch size={48} />
              <h2 className="text-lg font-medium mt-4">{t('COMMON.NO_SEARCH_RESULTS')}</h2>
              <p className="empty-text mt-2">{t('COOKIES.TRY_DIFFERENT_SEARCH')}</p>
            </div>
          ) : (
            // Show cookies list
                    ...
                    <Accordion.Header index={i} className="flex items-center">
                      <div className="flex items-center">
                        ...
                        <span className="domain-count ml-2 text-xs">
                          ...
                          {domainWithCookies.cookies.length === 1 ? t('COOKIES.COOKIE_SINGULAR') : t('COOKIES.COOKIE_PLURAL')})
                        </span>
