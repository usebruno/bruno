    ...
    return (
      <div ref={ref} className="flex items-center justify-center ml-2 cursor-pointer select-none" data-testid="grpc-method-dropdown-trigger">
        ...
        <span className="text-xs">
          {selectedGrpcMethod ? (
            <span className="method-dropdown-text text-nowrap" data-testid="selected-grpc-method-name">
              {selectedGrpcMethod.path.split('.').at(-1) || selectedGrpcMethod.path}
            </span>
          ) : (
            <span className="method-dropdown-text text-nowrap">Select Method </span>
          )}
        </span>
          ...
          <div ref={listRef} className="max-h-96 overflow-y-auto w-96 min-w-60" data-testid="grpc-methods-list">
            {Object.entries(groupedMethods).map(([serviceName, methods], serviceIndex) => (
              <div key={serviceIndex} className="service-group mb-2" onKeyDown={handleKeyDown} tabIndex={0}>
                <div className="service-header px-3 py-1 font-medium truncate sticky top-0 z-10">
                  ...
                  {methods.map((method, methodIndex) => {
                    ...
                    return (
                      <div
                        ...
                        className={`method-item py-2 px-3 w-full border-l-2 transition-all duration-200 relative group ${
                          selectedGrpcMethod && selectedGrpcMethod.path === method.path
                            ? 'border-yellow-500 selected'
                            : 'border-transparent'
                        } ${focusedIndex === globalMethodIndex ? 'focused' : ''}`}
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <div className="flex flex-col flex-1">
                            <div className="method-name font-medium">
                              {method.methodName}
                            </div>
