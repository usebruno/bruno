...
const Oauth2ActionButtons = ({ item, request, collection, url: accessTokenUrl, credentialsId }) => {
  ...
  const useSystemBrowser = get(preferences, 'request.useSystemBrowser', false);
  ...
  const handleFetchOauth2Credentials = async () => {
    ...
    if (fetchingAuthorizationCode) toggleFetchingAuthorizationCode(false);
  }
  ...
  const handleCancelAuthorization = async () => {
    try {
      const result = await dispatch(cancelOauth2AuthorizationRequest({ itemUid: item.uid }));
