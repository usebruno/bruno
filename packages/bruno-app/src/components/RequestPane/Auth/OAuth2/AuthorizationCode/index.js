...
const OAuth2AuthorizationCode = ({
  ...
  return (
    <StyledWrapper className="mt-2 flex w-full gap-4 flex-col">
            ...
            <label className="min-w-[140px] flex items-center gap-4">
              ...
              {tooltip && (
                <div className="relative group cursor-pointer inline-flex items-center">
                  ...
                  <span
                    className="group-hover:opacity-100 pointer-events-none opacity-0 max-w-60 absolute left-0 top-full mt-1 w-max p-2 text-xs rounded-md transition-opacity duration-200 z-10"
                    style={tooltipStyle}
                  >
                    {tooltip}
                  </span>
      ...
      <div
        className="flex items-center gap-4 w-full"
        key="input-credentials-placement"
      >
            ...
            items={[
              {
                id: 'body',
                label: 'Request Body',
                onClick: () => handleChange('credentialsPlacement', 'body')
              },
              {
      ...
      {tokenPlacement === 'header' ? (
        <div
          className="flex items-center gap-4 w-full"
          key="input-token-prefix"
          data-testid="token-header-prefix"
        >
        ...
        <div
          className="flex items-center gap-4 w-full"
          key="input-token-query-param-key"
          data-testid="token-query-param-key"
        >
      ...
      <div className="flex items-center gap-4 w-full">
        ...
        <div className="flex items-center gap-2">
          <div className="relative group cursor-pointer">
            <IconHelp size={16} className="text-gray-500" />
            <span
              className="group-hover:opacity-100 pointer-events-none opacity-0 max-w-60 absolute left-0 bottom-full mb-1 w-max p-2 text-xs rounded-md transition-opacity duration-200"
              style={tooltipStyle}
            >
              Automatically fetch a new token when you try to access a resource
              and don't have one.
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center gap-4 w-full">
        <input
          ...
          className={`cursor-pointer ml-1 ${
            isAutoRefreshDisabled ? 'opacity-50 cursor-not-allowed' : ''
          }`}
          disabled={isAutoRefreshDisabled}
        />
        <label
          className={`block min-w-[140px] ${
            isAutoRefreshDisabled ? 'text-gray-500' : ''
          }`}
        >
          Auto refresh token (with refresh URL)
        </label>
        <div className="flex items-center gap-2">
          <div className="relative group cursor-pointer">
            ...
            <span
              ...
              style={tooltipStyle}
            >
              Automatically refresh your token using the refresh URL when it
              expires.
            </span>
          </div>
        </div>
