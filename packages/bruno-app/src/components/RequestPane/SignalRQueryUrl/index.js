...
import { signalrConnectOnly, saveRequest } from 'providers/ReduxStore/slices/collections/actions';
...
import { getSignalRConnectionStatusFn, stopSignalRConnection } from 'utils/network/index';
...
const useSignalRConnectionStatus = (requestId) => {
  const [connectionStatus, setConnectionStatus] = useState(CONNECTION_STATUS.DISCONNECTED);
  useEffect(() => {
    const checkConnectionStatus = async () => {
      const result = await getSignalRConnectionStatusFn(requestId);
...
const SignalRQueryUrl = ({ item, collection, handleRun }) => {
  const dispatch = useDispatch();
  const { theme, displayedTheme } = useTheme();
  const saveShortcut = isMacOS() ? '\u2318S' : 'Ctrl+S';
  const hasChanges = useMemo(() => hasRequestChanges(item), [item]);

  const [connectionStatus, setConnectionStatus] = useSignalRConnectionStatus(item.uid);
  ...
  const handleConnect = async () => {
    dispatch(signalrConnectOnly(item, collection.uid));
  ...
  const handleDisconnect = async (e, notify) => {
    ...
    stopSignalRConnection(item.uid)
      .then(() => {
        notify && toast.success('SignalR connection closed');
        setConnectionStatus('disconnected');
      })
      .catch((err) => {
        console.error('Failed to close SignalR connection:', err);
        notify && toast.error('Failed to close SignalR connection');
      });
  };
  ...
  const handleReconnect = async (e) => {
    ...
    try {
      ...
      console.error('Failed to re-connect SignalR connection', err);
    }
  };
      ...
      <div className="flex items-center h-full">
        <div className="flex items-center input-container flex-1 min-w-0 h-full relative">
          <div className="flex items-center justify-center px-[10px]">
            <span className="text-xs font-medium method-signalr">SIGNALR</span>
          </div>
          <SingleLineEditor
            value={url}
...
export default SignalRQueryUrl;
