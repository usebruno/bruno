...
import WSStatusCode from '../WsResponsePane/WSStatusCode';
...
import WSResponseHeaders from '../WsResponsePane/WSResponseHeaders';

// Shared response-pane shell for long-lived, socket-backed request types
// (ws-request, graphql-subscription-request) — identical tab/status/timeline
// chrome, differing only in which messages-list component renders the
// "Messages" tab body.
const SocketResponsePane = ({ item, collection, MessagesList }) => {
  ...
  const getTabPanel = (tab) => {
    switch (tab) {
      case 'response': {
        return <MessagesList messages={response.responses || []} />;
      }
      case 'headers': {
...
export default SocketResponsePane;
