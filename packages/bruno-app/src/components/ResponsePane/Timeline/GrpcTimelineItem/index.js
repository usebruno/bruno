...
const EventTypeIcons = {
  ...
  response: <IconDatabase size={16} strokeWidth={1.5} className="text-green-500" />,
  request: <IconSend size={16} strokeWidth={1.5} className="text-orange-500" />,
  message: <IconSend size={16} strokeWidth={1.5} className="text-orange-500" />,
...
const EventTypeColors = {
  metadata: "border-blue-500/50",
  response: "border-green-500/50",
  request: "border-orange-500/50",
  message: "border-orange-500/50",
  status: "border-purple-500/50", 
  error: "border-red-500/50",
  end: "border-gray-500/50",
  cancel: "border-amber-500/50"
};
      ...
      case 'request':
        return (
          <div className="mt-2 bg-orange-50 dark:bg-orange-900/10 rounded p-2">
            <div className="font-semibold mb-1 text-orange-700 dark:text-orange-400">Request</div>
            
            {/* Method type badge - display client-streaming, server-streaming, etc. */}
            {effectiveRequest.methodType && (
              <div className="mb-2">
                <span className="px-2 py-1 text-xs rounded bg-orange-100 dark:bg-orange-800/30 text-orange-700 dark:text-orange-300">
                  {effectiveRequest.methodType}
                </span>
              </div>
            )}
            ...
            {effectiveRequest.headers && Object.keys(effectiveRequest.headers).length > 0 && (
              <div className="mb-3">
                <div className="text-xs font-medium mb-1 text-orange-700 dark:text-orange-400">Headers</div>
            ...
            {effectiveRequest.body?.mode === 'grpc' && effectiveRequest.body?.grpc?.length > 0 && (
              <div>
                ...
                <div className="space-y-2">
                  {effectiveRequest.body.grpc.filter((_, index) => index === 0).map((message, idx) => (
                    <div key={idx} className="bg-white dark:bg-gray-800 p-2 rounded">
                      <div className="text-xs font-medium mb-1">{message.name || `Message ${idx + 1}`}</div>
            ...
            {/* Fallback to show full request when the structure doesn't match */}
            {(!effectiveRequest.body?.mode || effectiveRequest.body?.mode !== 'grpc') && (
              <pre className="text-xs bg-white dark:bg-gray-800 p-2 rounded overflow-auto max-h-[200px]">
                {JSON.stringify(effectiveRequest, null, 2)}
              </pre>
            )}
          </div>
        );
        ...
        return (
          <div className="mt-2 bg-green-50 dark:bg-green-900/10 rounded p-2">
            <div className="font-semibold mb-1 text-green-700 dark:text-green-400">
              Response Message #{(response.messageCount || 0)}
            </div>
        ...
        return (
          <div className="mt-2 bg-gray-50 dark:bg-gray-700/30 rounded p-2">
            ...
            <div className="text-sm">
              Total messages: {response.messageCount || 0}
              {duration && <span className="ml-2">Duration: {duration}ms</span>}
            </div>
          </div>
        );
