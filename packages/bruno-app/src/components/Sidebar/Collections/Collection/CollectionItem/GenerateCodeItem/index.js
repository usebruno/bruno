...
const GenerateCodeItem = ({ collectionUid, item, onClose, isExample = false, exampleUid = null }) => {
  ...
  const modalTitle = isExample ? t('SIDEBAR_COLLECTIONS.GENERATE_CODE_EXAMPLE', { name: get(item, 'draft.examples', []).find((e) => e.uid === exampleUid)?.name || 'Example' }) : t('SIDEBAR_COLLECTIONS.GENERATE_CODE');
          ...
          <div className="editor-container">
            {validateURLWithVars(finalUrl) ? (
              ...
              <div className="error-message">
                <h1>{t('SIDEBAR_COLLECTIONS.INVALID_URL')}: {finalUrl}</h1>
                <p>{t('SIDEBAR_COLLECTIONS.CHECK_URL_TRY_AGAIN')}</p>
              </div>
            )}
          </div>
        </div>
