...
const getMethodFlags = (item) => ({
  ...
  isGraphQLSubscription: item.type === 'graphql-subscription-request'
});

const getMethodText = (item, { isGrpc, isWS, isGraphQL, isGraphQLSubscription }) => {
  if (isGrpc) return 'grpc';
  if (isWS) return 'ws';
  if (isGraphQL) return 'gql';
  if (isGraphQLSubscription) return 'sub';
  return item.request.method.length > 5
    ? item.request.method.substring(0, 3)
    : item.request.method;
};

const getClassname = (method = '', { isGrpc, isWS, isGraphQL, isGraphQLSubscription }) => {
  ...
  return classnames('mr-1', {
    ...
    'method-grpc': isGrpc,
    'method-ws': isWS,
    'method-graphql': isGraphQL || isGraphQLSubscription
  });
};
