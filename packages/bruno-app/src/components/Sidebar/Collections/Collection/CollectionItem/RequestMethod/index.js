...
const getMethodFlags = (item) => ({
  ...
  isSignalR: item.type === 'signalr-request'
});

const getMethodText = (item, { isGrpc, isWS, isGraphQL, isSignalR }) => {
  if (isGrpc) return 'grpc';
  if (isWS) return 'ws';
  if (isGraphQL) return 'gql';
  if (isSignalR) return 'signalr';
  return item.request.method.length > 5
    ? item.request.method.substring(0, 3)
    : item.request.method;
};

const getClassname = (method = '', { isGrpc, isWS, isGraphQL, isSignalR }) => {
