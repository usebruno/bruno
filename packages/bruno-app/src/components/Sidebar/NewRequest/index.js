...
const NewRequest = ({ collectionUid, item, isEphemeral, onClose }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      requestName: Yup.string()
        .trim()
        .min(1, t('SIDEBAR.NEW_REQUEST_MIN_1'))
        .max(255, t('SIDEBAR.NEW_REQUEST_MAX_255'))
        .required(t('SIDEBAR.NEW_REQUEST_NAME_REQUIRED')),
      filename: Yup.string()
        .trim()
        .min(1, t('SIDEBAR.NEW_REQUEST_MIN_1'))
        .max(255, t('SIDEBAR.NEW_REQUEST_MAX_255'))
        .required(t('SIDEBAR.NEW_REQUEST_FILENAME_REQUIRED'))
        ...
        .test(
          'not-reserved',
          t('SIDEBAR.NEW_REQUEST_RESERVED'),
      ...
      curlCommand: Yup.string().when('requestType', {
        ...
        then: Yup.string()
          .min(1, t('SIDEBAR.NEW_REQUEST_MIN_1'))
          .required(t('SIDEBAR.NEW_REQUEST_CURL_REQUIRED'))
          .test({
            name: 'curlCommand',
            message: t('SIDEBAR.NEW_REQUEST_INVALID_CURL'),
    ...
    onSubmit: (values) => {
      ...
      if (isGrpcRequest) {
        dispatch(
          ...
          .then(() => {
            toast.success(t('SIDEBAR.NEW_REQUEST_CREATED'));
            onClose();
          })
  ...
  return (
    <Portal>
      <StyledWrapper>
        <Modal size="md" title={t('SIDEBAR.NEW_REQUEST_TITLE')} hideFooter handleCancel={onClose}>
          <form
          ...
          >
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between">
                  <label htmlFor="filename" className="flex items-center font-medium">
                    {t('SIDEBAR.NEW_REQUEST_FILE_NAME')} <small className="font-normal text-muted ml-1">{t('SIDEBAR_COLLECTIONS.ON_FILESYSTEM')}</small>
                    <Help width="300">
                      <p>{t('SIDEBAR.NEW_REQUEST_FILE_NAME_DESC')}</p>
                      <p className="mt-2">
                        {t('SIDEBAR.NEW_REQUEST_FILE_NAME_DESC_2')}
                      </p>
                    </Help>
                  </label>
                  {isEditing ? (
            ...
            {formik.values.requestType !== 'from-curl' ? (
              ...
              <div className="mt-4">
                ...
                <textarea
                  name="curlCommand"
                  placeholder={t('SIDEBAR.NEW_REQUEST_ENTER_CURL')}
            ...
            <div className="flex justify-between items-center mt-8 bruno-modal-footer">
              <div className="flex advanced-options">
                <Dropdown onCreate={onAdvancedDropdownCreate} icon={<AdvancedOptions />} placement="bottom-start">
                  ...
                  >
                    {showFilesystemName ? t('SIDEBAR.NEW_REQUEST_HIDE_FILESYSTEM_NAME') : t('SIDEBAR.NEW_REQUEST_SHOW_FILESYSTEM_NAME')}
                  </div>
                </Dropdown>
              </div>
