...
const CreateApiSpec = ({ onClose }) => {
    ...
    if (collectionLocation) {
      ...
      ipcRenderer
        ...
        .catch((err) => {
          ...
          toast.error(t('SIDEBAR.FAILED_TO_LOAD_COLLECTION'));
        });
    }
  ...
  return (
    <StyledWrapper>
      <Modal size="md" title={t('SIDEBAR.CREATE_API_SPEC')} confirmText={t('COMMON.CREATE')} handleConfirm={onSubmit} handleCancel={onClose}>
          ...
          <div>
            ...
            <div className="flex items-center">
              ...
              <label htmlFor="collection" className="ml-1 cursor-pointer select-none">
                {t('SIDEBAR.FROM_BRUNO_COLLECTION')}
              </label>
            </div>
            ...
            <div className="mt-1">
              ...
              {!isDefaultWorkspace && (
                <span className="text-xs opacity-60 ml-2">
                  {t('SIDEBAR.DEFAULTS_TO_APISPEC_FOLDER')}
                </span>
              )}
            </div>
          </div>
