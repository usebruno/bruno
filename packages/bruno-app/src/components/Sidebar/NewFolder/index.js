  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      directoryName: Yup.string()
        .trim()
        .min(1, t('SIDEBAR.MUST_BE_AT_LEAST_1_CHAR'))
        .required(t('SIDEBAR.FOLDERNAME_REQUIRED'))
        ...
        .test({
          name: 'folderName',
          message: t('SIDEBAR.FOLDERNAME_ENVIRONMENTS_RESERVED'),
          test: (value) => {
    ...
    onSubmit: (values) => {
      dispatch(newFolder(values.folderName, values.directoryName, collectionUid, item ? item.uid : null))
        ...
        .catch((err) => toast.error(err ? err.message : t('SIDEBAR.ERROR_ADDING_FOLDER')));
    }
  });
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between">
                  <label htmlFor="directoryName" className="flex items-center font-medium">
                    {t('SIDEBAR.FOLDER_NAME')} <small className="font-normal text-muted ml-1">{t('SIDEBAR.ON_FILESYSTEM')}</small>
                    <Help width="300">
                      <p>
