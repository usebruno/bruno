...
const randomChallenge = () => crypto.randomBytes(8);
 ...
 * nonce is fresh per call because the client keys its proof by it, so a proof answering one
 * connection cannot be replayed onto another, which is what binds a handshake to its connection.
 */
const type2Message = (challenge = randomChallenge()) => {
...
module.exports = { type2Message, messageType, provesPassword, randomChallenge };
